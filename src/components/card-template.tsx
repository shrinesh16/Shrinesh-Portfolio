...
const CardTemplate = forwardRef<CardTemplateRef, CardTemplateProps>(
  ({ userName, variant, onTextureReady, city, date, cardImageUrl }, ref) => {
    ...
    // Preload the card image if provided
    useEffect(() => {
      if (!cardImageUrl) {
        setCardImage(null);
        return;
    ...
    const captureTexture = async () => {
      ...
      // If card image is provided, use it as the base
      if (cardImage) {
        // Draw the card image covering the entire canvas with proper aspect ratio
        const imgWidth = cardImage.width;
        const imgHeight = cardImage.height;
        const imgAspect = imgWidth / imgHeight;
        const canvasAspect = CANVAS_SIZE / CANVAS_SIZE; // 1:1 for square canvas
    ...
    const exportCard = () => {
      ...
      if (cardImage) {
        ...
        fullCtx.drawImage(cardImage, drawX, drawY, drawWidth, drawHeight);
      } else {
        // Draw base card image (fills entire canvas)
        if (baseImage) {
